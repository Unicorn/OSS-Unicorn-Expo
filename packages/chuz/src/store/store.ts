...
const setItem = async <T>(key: string, value: T): Promise<void> => {
  try {
    await AsyncStorage.setItem(`elevate_${key}`, JSON.stringify(value));
  } catch (e) {
...
const getItem = async <T>(key: string, defaultValue?: T | null): Promise<T | null> => {
  try {
    const value = await AsyncStorage.getItem(`elevate_${key}`);
...
const removeItem = async (key: string): Promise<void> => {
  try {
    await AsyncStorage.removeItem(`elevate_${key}`);
  } catch (e) {
